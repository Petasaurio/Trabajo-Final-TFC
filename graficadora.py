...
# Funcion para generar una lista de colores random usada para graficar las distintas curvas
...
# Funcion para graficar los datos de un fichero dado. Valores de opcion consistentes con los de funcion graficar(), y RP necesario solo si opcion == 1

def plotFichero(fichero, opcion, RP):
    ...
    if opcion == 0:                         # segun la opcion que sea, realizar la grafica correspondiente
...
def graficar(opcion):  # valores de opcion:
                       ...
                       #   3 -> grafica de intervalos de tiempo entre 0 y 3 meses
...
# Para opciones 0 y 1, se grafica entre -3pi/4 y 3pi/4. Para opciones 2 y 3 entre -4pi/5 y 4pi/5 para que los puntos graficados queden sobre la curva
...
# Se grafica la trayectoria del cometa C/1823 Y1
...
# Segun la opcion que sea, se crean las tablas de datos correspondientes y se grafican usando 'plotFichero()'
    if opcion == 1:
        for n in range (0, 5):
